...
SELECT NOMBRE,
  ...
  APE2
FROM SOCIOS
WHERE APE1 NOT LIKE 'RAMIREZ'
INTERSECT
SELECT NOMBRE,APE1,APE2 FROM SOCIOS WHERE APE1 NOT LIKE 'RODRIGUEZ';
-- 83 NOMBRE DE LOS SOCIOS QUE NO REALIZAN MAS DE 2 ALQUILERES
SELECT NOMBRE
FROM SOCIOS
WHERE C_SOCIO NOT IN
  (SELECT C_SOCIO FROM ALQUILER GROUP BY C_SOCIO HAVING COUNT(*)>2
  );
...
FROM VENTAS,
  ARTICULO
WHERE VENTAS.C_ART      = ARTICULO.C_ART
AND SUBSTR(FECHA, 7, 2) = '17'
ORDER BY DESCRIP;
...
-- ARTICULOS QUE VALEN MAS DE 10 EUROS
SELECT DESCRIP
FROM ARTICULO
WHERE PRECIO > 10;
-- ARTICULOS QUE VALEN 11 EUROS
SELECT DESCRIP
FROM ARTICULO
WHERE PRECIO = 11;
-- ARTICULOS CON PRECIO ENTRE 8 Y 11
SELECT DESCRIP
FROM ARTICULO
WHERE PRECIO > 8
AND PRECIO   < 11;
-- CLIENTES QUE TENGAN EN EL COMENTARIO 'BUEN'
SELECT NOMBRE
FROM CLIENTE
WHERE COMENTARIO LIKE 'BUEN%';
-- CLIENTES QUE TIENEN TELEFONO DE SANTANDER 978------
SELECT NOMBRE
FROM CLIENTE
WHERE TELEF LIKE '978%';
-- IDPED E IDCLI DE LOS PEDIDOS REALIZADOS ENTRE EL 1 DE ENERO Y 30 DE JUNIO DE 2015
SELECT IDPED,
  IDCLI
FROM PEDIDO
WHERE FPED < '30/6/15'
AND FPED   > '1/1/15'; 
-- IDPED DE DETALLE QUE SEAN IGUALES A IDPED
SELECT DISTINCT IDPED FROM DETALLE WHERE IDPED IN (SELECT IDPED FROM PEDIDO);
-- CODIGO DE ARTICULO Y CODIGO DE FACTURA DEL CLIENTE QUE SE LLAMA ZAMORANO
SELECT C_FACTURA, C_CLIENTE FROM VENTAS WHERE C_CLIENTE IN 
(SELECT C_CLIENTE FROM CLIENTES WHERE NOMBRE = 'Zamorano');
-- ARTICULOS CON SU CODIGO DEL PRIMER SEMESTRE DEL CLIENTE MARIANITO
SELECT DESCRIP, C_ART FROM ARTICULO WHERE C_ART IN 
(SELECT C_ART FROM VENTAS WHERE C_CLIENTE = 
(SELECT C_CLIENTE FROM CLIENTES WHERE NOMBRE = 'Marianito')
AND
FECHA > '1/1/12'
AND
FECHA < '30/6/12'
);
-- CANTIDAD DE PRODUCTOS QUE VALGAN 25e
SELECT COUNT(*) FROM ARTICULO WHERE PRECIO = 25;