--1 TITULO DE LAS PELICULAS PRODUCIDAS EN A�OS DIFERENTES A 2007
SELECT TITULO
FROM PELICULAS
WHERE ANNO = 2000;
...
--5 DE QUE A�O ES LA PELICULA MAS ANTIGUA
...
--12 FECHA DEL �LTIMO ALQUILER
SELECT MAX(FECHA) FROM ALQUILER;
--13 A�O DE LA PEL�CULA M�S ANTIGUA QUE EST� EN OFERTA
SELECT MAX(ANNO)
FROM PELICULAS
WHERE OFERTA='S';
...
--22 TITULO DE LA PELICULA M�S ANTIGUA
SELECT TITULO,
  ANNO
FROM PELICULAS
WHERE ANNO =
...
--23 �CUAL ES EL PA�S QUE M�S CINES TIENE?
SELECT NOMBRE_PAIS
FROM PAISES
WHERE CINES =
...
--26 TITULO DE LAS PELICULAS ESPA�OLAS
SELECT TITULO
FROM PELICULAS
WHERE C_PAIS =
  (SELECT C_PAIS FROM PAISES WHERE NOMBRE_PAIS = 'Espa�a'
  );
...
--36 NOMBRE DE LOS SOCIOS QUE NO HAN ALQUILADO PELICULAS ESPA�OLAS
SELECT NOMBRE
FROM SOCIOS
WHERE C_SOCIO NOT IN
  (SELECT C_SOCIO
...
--37 NOMBRE DE LOS SOCIOS QUE VEN PELICULAS QUE NO SON ESPA�OLAS
SELECT NOMBRE
FROM SOCIOS
WHERE C_SOCIO IN
  (SELECT C_SOCIO
...
--38 VISUALIZA CUANTS PELICULAS SE HAN EREALIZADO EN LOS DISTINTOS A�OS
SELECT ANNO,
  COUNT(C_PELI)
FROM PELICULAS
GROUP BY ANNO
...
FROM PELICULAS
GROUP BY FORMATO;
--40 VISUALIZA CUANTAS PELICULAS SE HAN ALQUILADO EN LOS DISTINTOS A�OS
SELECT '20'
  ||SUBSTR(FECHA, 7, 2),
  COUNT(*)
FROM ALQUILER
...
-- 48 �CUANTAS PELICULAS ESPA�OLAS HA ALQUILADO ROSA?
SELECT COUNT(*)
FROM PELICULAS
WHERE C_PAIS =
  (SELECT C_PAIS FROM PAISES WHERE NOMBRE_PAIS = 'Espa�a'
...
-- 50 LISTADO CON LOS NOMBRES DE LOS PAISES JUNTO CON LOS A�OS
...
-- 54 ¿CUANTOS SOCIOS VIVEN EN SOL?
SELECT COUNT(*)
FROM CLIENTE
...
-- 56 ¿CUANTOS PEDIDOS NOS HA REALIZADO MARIA PEREZ?
SELECT COUNT(*)
FROM PEDIDO
WHERE IDCLI =
...
GROUP BY FORMATO
HAVING COUNT(*) > 6;
-- 69 �CUAL ES EL FORMATO M�S FRECUENTE?
SELECT FORMATO
FROM PELICULAS
GROUP BY FORMATO
HAVING COUNT(*) =
...
-- 70 �CUAL ES EL A�O EN EL QUE SE REALIZARON M�S PELICULAS?
SELECT ANNO
FROM PELICULAS
GROUP BY ANNO
HAVING COUNT(*) =
...
-- 72 �EN QUE A�O SE ALQUILARON MAS PELICULAS?
...
-- 75 NOMBRE DE LOS SOCIOS QUE NO VEN PELICULAS ESPA�OLAS
SELECT NOMBRE
FROM SOCIOS
WHERE C_SOCIO NOT IN
  (SELECT C_SOCIO
...
SELECT TITULO FROM PELICULAS WHERE C_PELI IN
(SELECT C_PELI FROM ALQUILER GROUP BY C_PELI HAVING SUM(TOTAL) = (SELECT MAX(SUM(TOTAL)) FROM ALQUILER GROUP BY C_PELI));
-- 79 �CUANTO SE HA FACTURADO CON LAS PELICULAS ESPA�OLAS?
SELECT SUM(SUM(TOTAL)) FROM ALQUILER GROUP BY C_PELI HAVING C_PELI IN 
(SELECT C_PELI FROM PELICULAS WHERE C_PAIS = (SELECT C_PAIS FROM PAISES WHERE NOMBRE_PAIS = 'Espa�a'));
-- 80 VISUALIZA LOS NOMBRES DE LOS PAISES QUE NO TIENEN NINGUN ALQUILER
SELECT NOMBRE_PAIS FROM PAISES WHERE C_PAIS NOT IN 
(SELECT C_PAIS FROM PELICULAS WHERE C_PELI IN (SELECT C_PELI FROM ALQUILER));
-- 81 NOMBRE DE LOS SOCIOS QUE SON DE MADRID O DE TOLEDO (POR SUS TELEFONOS)
SELECT NOMBRE AS MADRID_Y_TOLEDO FROM SOCIOS WHERE TELEFONO LIKE '91%'
UNION
